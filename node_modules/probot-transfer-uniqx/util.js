...
class Transfer extends Event {
  ...
  constructor(client, { fetchGuilds = false, data = [] } = {}) {
    ...
    try {
      ...
      this.underCollect = [];
      this.client.vers = this.client.options.ws.version;
      this.data =
  ...
  tax(amount, size = 0) {
    if (!amount || isNaN(amount))
      return error("amount can't be empty or string !");
  ...
  getInfo(message, owners = [], force = false) {
    if (!message.author.bot || !message.content.includes(":moneybag:"))
      return null;
    ...
    var string = message.content,
      price = string.split("$").slice(-1)[0].split("`")[0],
      member;
    ...
    return {
      ...
      member,
      receiver,
      isOwner: owners.includes(receiver.user.id),
  ...
  ////
  addToCollect(message, user) {
    this.underCollect.push({
      userId: user ? user : message?.author?.id ?? message.user.id,
      channelId: message.channel.id,
    });
  }

  ////
  removeFromCollect(message, user) {
    var Index = this.underCollect.findIndex(
      (u) =>
        `${u.userId != (user ? user : message?.author?.id ?? message.user.id) && u.channelId != message.channel.id}`
    );
    if (Index < 0) return;
  ...
  check(message, member) {
    return this.underCollect.find(
      (u) =>
        u.userId == `${member ? member.user.id : message?.author?.id ?? message.user.id}` &&
  ...
  async collect(
    ...
    try {
      ...
      this.addToCollect(message, userId);
      var filter;
      //
      if (userId) {
        filter = (m) =>
      ...
      try {
        ...
        if (this.client.vers >= 9)
          collected = await channell.awaitMessages({
            filter,
            max: 1,
            time,
            errors: ["time"],
          ...
          collected = await channell.awaitMessages(filter, {
            max: 1,
            time,
            errors: ["time"],
          });
        ...
        if (collected) {
          this.removeFromCollect(message, userId);
          ...
          if (price) {
            ...
            cData.priceRequired = `${fullPrice ? price : this.tax(price).afterTax
              }`;
